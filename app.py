...
try:
    ...
    st.sidebar.success("✅ API Gemini đã kết nối")
...
if menu == "📋 Quản lý nhân viên":
    ...
    if search:
        df = st.session_state.employees[
            st.session_state.employees["ho_ten"].str.contains(
    ...
    if not st.session_state.employees.empty:
        ...
        "Chức vụ",
        positions,
        index=positions.index(emp["chuc_vu"]) if emp["chuc_vu"] in positions else 0
        )
...
# =====================================================
    ...
    if st.button("🚀 Phân tích CV bằng AI") and jd and uploaded_cv:
        ...
        try:
            ...
            if text.strip() == "":
                st.error("Không đọc được nội dung CV")
                st.stop()

        except Exception as e:
            st.error(f"Lỗi đọc PDF: {str(e)}")
            st.stop()

        with st.spinner("Gemini đang phân tích CV..."):
